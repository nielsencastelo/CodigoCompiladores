...
class Parser {  
  ...
  {  
    ...
    if(tokType == VARIABLE) {  
      // salva o token antigo 
      temptoken = new String(token);  
      ttokType = tokType;  
  
      // Calcula o indice da variável.  
      varIdx = Character.toUpperCase(token.charAt(0)) - 'A';  
  
      getToken();  
      if(!token.equals("=")) {  
        putBack(); // returna o token atual
        // restaura token antigo - não uma atribuição  
        token = new String(temptoken);  
        tokType = ttokType;  
      }  
      else {  
        getToken(); // obtém a próxima parte da expressão 
        result = evalExp2();  
        vars[varIdx] = result;  
        return result;  
      }  
